...
import UserPaginatedTable from "@/components/atoms/UserPaginatedTable";
...
const UsersPage = () => {
  ...
  const [userStaff, setUserStaff] = useState<any[]>([]);
  ...
  const handleOpenModal = (id: string, newData: any) => {
  ...
  const handleUpdate = () => {
    ...
    for (const key in userToUpdate) {
      ...
      if (!userToUpdate[key as keyof any]) {
        toast.error(`${key} field cannot be empty`);
        return;
      }
    }
    ...
    return (
      <>
        ...
        <main className="py-8">
          ...
          <div className="flex justify-end gap-8 items-center my-4">
            <button
              onClick={openModal}
